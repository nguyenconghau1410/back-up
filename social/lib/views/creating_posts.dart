...
import 'package:social/services/checkbox_controller.dart';
import 'package:social/utils/video_app.dart';
import 'package:social/views/next_creating_post.dart';
...
class _CreatingPostsState extends State<CreatingPosts> {
  ...
  Future<List<FileUtils>> _loadImages() async {
    ...
    if(albums.isNotEmpty) {
      List<AssetEntity> assets = await albums.first.getAssetListRange(start: 0, end: 12);
      List<FileUtils> files = [];
      for(var x in assets) {
        if(x.type == AssetType.image) {
          files.add(FileUtils(await x.file, "IMAGE"));
        }
        else if(x.type == AssetType.video) {
          files.add(FileUtils(await x.file, "VIDEO"));
        }
                ...
                return Expanded(child: GridView.count(
                  ...
                  children: List.generate(data!.length, (index)  {
                    if(data[index]!.type == "VIDEO")  {
                      VideoUtils videoUtils = VideoUtils(data[index]!.file!);
                      return Obx(
                          () => onClick == true
                              ? InkWell(
                                  onTap: () {
                                        ...
                                        child: Theme(
                                          ...
                                          child: Obx(
                                              ...
                                              onChanged: (value) {
                                                ...
                                                print(_lists[index].isCheck.value);
                                                print(_lists.length);
                                                if(!_lists[index].isCheck.value) {
                                                  selectedFile!.add(data[index]!);
                                                }
                                                else {
                                                  selectedFile!.removeWhere((element) => element == data[index]!);
                                                }
                                              },
                                            ),
                                          ),
