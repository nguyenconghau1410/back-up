...
class _NextCreatingPostState extends State<NextCreatingPost> {
  ...
  Future<void> creatingPost() async {
    if(widget.file != null) {
      ...
      String? url = await FileUpload.uploadImage(widget.file!.file!, type);
      List<Images> images = [];
      if(url != null) {
        images.add(Images(null, url));
      }
      Post post = Post(null, Utils.user!.id, _controller.text,
          "Public", null, DateTime.now().toString(), null, "POST", images);
      ...
      for(int i = 0; i < widget.files!.length; i++) {
        String type = "";
        if(widget.files![i].type == "IMAGES") {
          type = "images";
        }
        else {
          type = "videos";
        }
        String? url = await FileUpload.uploadImage(widget.files![i].file!, type);
        if(url != null) {
          images.add(Images(null, url));
        }
      }
